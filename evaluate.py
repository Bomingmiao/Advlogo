...
def generate_labels(evaluator, cfg, args, save_label=False):
    ...
    if args.use_defense:
        from SAC.patch_detector import PatchDetector #defense
        SAC_processor = PatchDetector(3, 1, base_filter=16, square_sizes=[150, 100, 75, 50, 25], n_patch=1)
        SAC_processor.unet.load_state_dict(torch.load("./SAC/ckpts/coco_at.pth", map_location=evaluator.device))
        SAC_processor.to(evaluator.device)
    ...
    for index, img_batch in enumerate(tqdm(data_loader, total=len(data_loader))):
        ...
        for detector in evaluator.detectors:
            ...
            if args.use_defense:
                with torch.no_grad():
                    adv_img_tensor, _, _ = SAC_processor(adv_img_tensor, bpda=True, shape_completion=False)
                    adv_img_tensor = torch.stack(adv_img_tensor)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ud', '--use_defense', help='to use_defense', action='store_true')
