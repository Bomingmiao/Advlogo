...
class PatchRandomApplier(nn.Module):
    ...
    def forward(self, img_batch: torch.Tensor, adv_patch: torch.Tensor, bboxes_batch: torch.Tensor):
        ...
        if pad_size>0:
            ...
            adv_batch = adv_batch.view(batch_size*lab_len, adv_batch.size(2),adv_batch.size(3),adv_batch.size(4))
            adv_batch = F.interpolate(adv_batch, (img_batch.size(-2),img_batch.size(-1)))
            adv_batch = adv_batch.view(batch_size, lab_len, adv_batch.size(1),adv_batch.size(2),adv_batch.size(3))
