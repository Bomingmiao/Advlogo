    ...
    def attack(self, img_tensor_batch, mode='sequential'):
        ...
        if mode == 'optim' or mode == 'sequential':
            for detector in self.detectors:
                loss, det_loss = self.attacker.non_targeted_attack(img_tensor_batch, detector)
                detectors_loss.append(loss)
                detectors_loss_det.append(det_loss)
        elif mode == 'parallel':
            detectors_loss = self.parallel_attack(img_tensor_batch)
        self.attacker.end_attack()
        return torch.tensor(detectors_loss).mean(), torch.tensor(detectors_loss_det).mean()
