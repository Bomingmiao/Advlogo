...
class DiffAttack(Optimizer):
    ...
    def __init__(self, loss_func, norm: str, cfg, device: torch.device, detector_attacker):
        ...
        self.optimizer = torch.optim.AdamW([detector_attacker.patch_obj.uncond_embeddings_list[detector_attacker.patch_obj.start_step]],lr=0.01)
    ...
    def patch_update(self, **kwargs):
        if self.patch_obj.optimize == "latent":
            update = self.patch_obj.latent_init_fft.grad.float().sign()
            ...
            #eta = eta.float()
            #torch.clamp_(eta.data, min=-100, max=100)
            #latent_tmp = self.patch_obj.latent_init_fft_ori + eta
            #print(f'latent的均值{latent_tmp.mean().item()},标准差{latent_tmp.std().item()}')
            #torch.save(latent_tmp.detach().cpu(),'latent_tmp.pt')
